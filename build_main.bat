...
if errorlevel 1 echo [WARN] cryptography is not installed.
...
py -3 -m PyInstaller --noconfirm --onefile --windowed --name "%APP_NAME%" %ICON_ARG% ^
  --add-data "icon;icon" ^
  --add-data "shared;shared" ^
  --add-data "Adapters;Adapters" ^
  --add-data "Sets;Sets" ^
  --add-data "Matrix;Matrix" ^
  --add-data "Validator;Validator" ^
  --hidden-import shared.dialogs ^
  --hidden-import shared.theme_toggle ^
  --hidden-import shared.larix_api ^
  --hidden-import shared.excel_parser ^
  --hidden-import shared.excel_template ^
  --hidden-import shared.adapter_excel ^
  --hidden-import shared.adapter_picker ^
  --hidden-import shared.guide ^
