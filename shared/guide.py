...
_GUIDES = {'adapters': {'title': 'Справка — Адаптеры',
              ...
              'rules': ('Итоговый код формируется как «Группа параметров.Наименование параметра». Например BIM + '
                        ...
                        'Если несколько строк имеют одинаковое значение в «Параметры», приложение предупредит о '
                        'конфликте; при импорте будет использована последняя строка.',
 ...
 'matrix': {'title': 'Справка — Матрицы',
            ...
            'rules': ('Имена в строках и столбцах матрицы должны совпадать с «Имя набора» на выбранном листе наборов.',
                      ...
                      '.cv-файла.'),
            'tips': ('По умолчанию лист наборов можно брать из той же книги, что и матрицы. Старый вариант с отдельным Excel-файлом наборов тоже поддерживается — укажите его в поле «Excel-файл наборов».',
                     'Параметр фильтра можно выбрать из адаптера. В демо для этого есть «Классификатор.Код элемента».',
